...
export const getDoctorSchedules = async (doctorId: string) => {
  const res = await axiosInstance.get(
    `/DoctorSchedule/doctorSchedules?DoctorId=${doctorId}`,
  );
  return res.data;
};
...
export const addDoctorSchedule = async (scheduleData: {
  ...
  const res = await axiosInstance.post("/DoctorSchedule/Add", formData, {
    headers: {
...
export const deleteDoctorSchedule = async (id: number) => {
  const res = await axiosInstance.delete(`/DoctorSchedule/${id}`);
  return res.data;
};
...
export const updateBookingStatus = async (
  ...
  const res = await axiosInstance.post(
    `/Booking/change-status?Id=${bookingId}&status=${status}`,
  );
  return res.data;
};
...
export const getDoctorDashboardData = async () => {
  ...
  const totalPatients = new Set(
    bookings.map((b: any) => b.patientId || b.userId),
  ).size;
  ...
  const revenue = bookings.reduce(
    (sum: number, b: any) => sum + (b.price || b.amount || 0),
    0,
  );
  return {
    success: true,
    data: {
      ...
      revenue,
      upcomingAppointments: bookings.slice(0, 5),
      bookings,
    },
  };
};
...
export const getUpcomingAppointments = async () => {
  ...
  const upcoming = (res?.data || []).filter(
    (b: any) => new Date(b.bookingDate) >= new Date(),
  );
  return { success: true, data: upcoming };
};

export const getChatHistory = async (patientId: string) => {
  const res = await axiosInstance.get(`/Chat/history?patientId=${patientId}`);
  return res.data;
};
