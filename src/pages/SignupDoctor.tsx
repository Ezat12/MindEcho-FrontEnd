...
import { Mail, Lock, User, FileText, Activity, AlertCircle, ArrowRight, Calendar } from 'lucide-react';
...
export default function RegisterDoctor() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.success && res?.data?.token) {
        login(res.data.token, 'doctor', res.data.doctorId || '');
  ...
  return (
    <div className="min-h-screen bg-white text-black flex flex-col justify-center items-center p-4 md:p-8 font-sans" dir="ltr">
      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl p-6 md:p-8 border border-gray-200 shadow-md space-y-6 my-8">
        ...
        <div className="flex flex-col items-center text-center space-y-3">
          ...
          <div className="space-y-1">
            <h1 className="text-2xl font-bold text-black">Doctor Professional Sign Up</h1>
            <p className="text-xs text-gray-500">Create your medical practitioner account and connect with MindEcho.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-black mb-2">Medical License Number</label>
              <div className="relative">
          ...
          <div>
            <label className="block text-xs font-medium text-black mb-2">Medical Specialization Field</label>
            <div className="relative">
          ...
          <div>
            <label className="block text-xs font-medium text-black mb-2">Professional Biography (Bio)</label>
            <textarea
          ...
          >
            {loading ? 'Creating Professional Account...' : 'Sign Up & Continue'}
          </button>
        </form>
