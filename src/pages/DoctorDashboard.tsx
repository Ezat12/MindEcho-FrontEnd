      ...
      <main className="max-w-[1440px] mx-auto p-6">
        <header className="flex justify-between mb-10">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => navigate("/doctor-about")}
              className="px-5 py-3 bg-white border rounded-xl flex items-center gap-2"
            >
          ...
          <div className="col-span-8 bg-white p-8 rounded-3xl">
            ...
            {loading ? (
              ...
              upcoming.map((app: any) => (
                <div
                  key={app.id}
                  className="flex justify-between p-4 bg-gray-50 rounded-xl mb-3"
                >
                  <div>
...
const NavLink = ({ label, active, onClick }: any) => (
  <button
    onClick={onClick}
    className={`px-4 py-2 rounded-xl font-bold ${active ? "bg-blue-600 text-white" : ""}`}
  >
    {label}
  </button>
);
